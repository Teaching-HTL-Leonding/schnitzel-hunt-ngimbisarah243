...
void CheapestAndMostExpensiveFoods(string[][] meals, out string[] min, string[] datas, out string[] files, out string maxSeitanSchnitzel)
{
    min = new string[4]; files = new string[5];
    min[SEITANSCHNITZEL] = maxSeitanSchnitzel = meals[SEITANSCHNITZEL][0];
    min[APPETIZER] = meals[APPETIZER][0]; min[MAINDISH] = meals[MAINDISH][0]; min[DESSERT] = meals[DESSERT][0];
    files[0] = files[1] = files[2] = files[3] = files[4] = string.Empty;

    int indexCategory = 0;

    foreach (string[] category in meals)
    {
        foreach (string singleMeal in category)
        {
            var parts = singleMeal.Split(":");
            var partsMin = min[indexCategory].Split(":");
            
            if (int.Parse(parts[1][..^1]) < int.Parse(partsMin[1][..^1]))
            {
                min[indexCategory] = singleMeal;
            }
